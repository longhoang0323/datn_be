...
//    Page<Phong> getRoomByCheckDate(Pageable pageable, LocalDateTime checkIn, LocalDateTime checkOut);
    ...
    @Query("select p from Phong p " +
            "inner join ChiTietPhong ct on p.id = ct.phong.id " +
            ...
            "       (cast(d.checkIn as date) > cast(:checkIn as date) and cast(d.checkIn as date) < cast(:checkOut as date)) or " +
            "       (cast(d.checkOut as date) > cast(:checkIn as date) and cast(d.checkOut as date) < cast(:checkOut as date)) or " +
            "       cast(:checkIn as date) = cast(d.checkIn as date) or " +
            "       cast(:checkOut as date) = cast(d.checkOut as date)" +
            "   )" +
            ")")
    Page<Phong> getRoomByCheckDate(Pageable pageable,
